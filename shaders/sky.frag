...
uniform float luminance; // Borde inte vara valbar!
...
const float NAir = 2545E25; // Number of molecules per unit volume for Air
...
// Represents 66 arc seconds, converted to degrees, and then the cosine
...
// TODO: Undersök detta!!
// Earth shadow hack (TODO)
// cutoffAngle = PI / 1.95;
const float cutoffAngle = 1.6110731556870734;
const float steepness = 1.5;
const float EE = 1000.0; // TODO: Change this in settings! As Exposure!!! 
// TODO: compare to Tyro's hack
float sunIntensity( float zenithAngleCos ) {
  zenithAngleCos = clamp( zenithAngleCos, -1.0, 1.0 );
  return EE * max( 0.0, 1.0 - pow( ex, -( ( cutoffAngle - acos( zenithAngleCos ) ) / steepness ) ) );
}

// TODO: What's this? Vad kommer skiten ifrån? Ingen som nämnt turbinity alls än....
vec3 totalMie( float Turb ) {
  float c = ( 0.2 * Turb ) * 10E-18;
  return 0.434 * c * MieConst;
}

// Function to get a tonemapping with a filmic feel 
...
void main() {
  ...
  // Compare to other intensity functions!
  float sunExposure = sunIntensity( dot(sunDir, upDir) );

  // Ta in 450000 som sunDist, uniform!?
  float sunFade = 1.0 - clamp( 1.0 - exp(sunPos.y / 450000.0), 0.0, 1.0 );

  // TODO!!! What is rayleigh here? Undersök denna! (Varför gånger 1? Ändra? Byt namn på settings iaf!)
  ...
  // He uses 0.25 = 25% the way up we have our average density. 

  // Cutoff angle at 90 to avoid singularity in next formula.
	float zenithAngle = acos( max( 0.0, dot( upDir, viewDir ) ) );
	float denom = 1.0 / ( cos( zenithAngle ) + 0.15 * pow( 93.885 - ( ( zenithAngle * 180.0 ) / PI ), -1.253 ) ); // Massa konstiga kontanter!?
  ...
  // For Rayleigh the g constant can be omitted for an approximation.
	float rayPhase = rayleighPhase( cosTheta * 0.5 + 0.5 ); // Why this recalculation?
	vec3 betaRayTheta = totalRayleighCoeff * rayPhase;

  // For Mie phase the g constant (mieScatteringDir) should be between [-0.75, -0.999]
  ...
  vec3 accumulatedScattering =  (betaRayTheta + betaMieTheta) / (totalRayleighCoeff + totalMieCoeff);
	vec3 inLight = pow( sunExposure * accumulatedScattering * (1.0 - totExtinction), vec3(1.5) );
	inLight *= mix( vec3(1.0), pow( sunExposure * accumulatedScattering * totExtinction, vec3(0.5) ), 
                  clamp( pow( 1.0 - dot(upDir, sunDir), 5.0 ), 0.0, 1.0 ) );

  // Add some Nightsky ( TODO: Don't think this works anymore...)
	float theta = acos( viewDir.y ); // inclination/elevation --> y-axis, [-pi/2, pi/2]
	float phi = atan( viewDir.z, viewDir.x ); // azimuth --> x-axis ]-pi/2, pi/2[
  ...
  vec3 lightSolarDisc = vec3(0.1) * totExtinction;
	...
	lightSolarDisc += ( sunExposure * 19000.0 * totExtinction ) * sundisc;


  // Continuing filmicworlds example for a tonemap (HDR hack)
  ...
  vec3 whiteScale = 1.0 / Uncharted2Tonemap(vec3(1000.0)); //Pre-calc: 1.0748724675633854;
